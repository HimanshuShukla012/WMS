...
export const FinanceTab: React.FC<FinanceTabProps> = ({
  ...
  const filteredWaterFeeSummary =
    filterWaterFeeSummaryByLocation(waterFeeSummaryData);
      ...
      <div className="bg-white rounded-xl shadow-lg p-6">
        <div className="text-center py-12">
          ...
          <h4 className="text-lg font-medium text-gray-600 mb-2">
            No Water Fee Summary Data
          </h4>
          <p className="text-gray-500">
            No water fee summary records found for the selected criteria. Try
            adjusting your filters or check if data is available for the
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg p-6">
      ...
      <div className="mb-4 text-sm text-gray-600">
        Showing {filteredWaterFeeSummary.length} water fee records for{" "}
        {selectedLocationName}• Period:{" "}
        ...
        <table className="w-full">
          ...
          <tbody className="divide-y divide-gray-200">
            {filteredWaterFeeSummary.slice(0, 50).map((fee, index) => (
              <tr
                key={`${fee.DistrictName}-${fee.VillageName}-${fee.Year}-${fee.Month}-${index}`}
              ...
              >
                ...
                <td className="px-4 py-3 text-sm text-orange-600 text-right font-medium">
                  {(fee.OutstandingAmount || 0).toLocaleString()}
                </td>
              </tr>
            ))}
          </tbody>
