...
export const ChartsSection: React.FC<ChartsSectionProps> = ({
  ...
  const exportChartToPDF = async (chartRef: React.RefObject<HTMLDivElement>, filename: string) => {
    ...
    try {
      ...
      // Add title
      pdf.setFontSize(16);
      pdf.text(filename.replace(/_/g, ' ').toUpperCase(), 148, 15, { align: 'center' });
      ...
      // Add timestamp
      pdf.setFontSize(10);
      pdf.text(`Generated on: ${new Date().toLocaleString('en-IN')}`, 10, imgHeight + 35);
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-1 gap-6">
      ...
      <div className="bg-white rounded-xl shadow-lg p-6" ref={beneficiaryChartRef}>
  ...
  <div style={{ height: 280 }}>
    <ResponsiveContainer width="100%" height="100%">
      <BarChart data={beneficiaryTrend} margin={{ top: 25, right: 10, left: 0, bottom: 5 }}>
  ...
  <div className="bg-white rounded-xl shadow-lg p-6" ref={feeCollectionChartRef}>
    <div className="flex items-center justify-between mb-4">
      <h4 className="text-lg font-semibold text-gray-800 flex items-center gap-2">
